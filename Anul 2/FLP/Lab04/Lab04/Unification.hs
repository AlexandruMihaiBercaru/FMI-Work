...
class SubstitutionLike s where
  ...
  -- | Composition / combin s1ation of substitutions
  -- (s1 >>> s2) x means that first we apply on x then s2 on the result
  (>>>) :: s -> s -> s
...
unifyStep (Config [] substitutions) = Right (Config [] substitutions)
unifyStep (Config (eq:restEqList) substitutions) 
  | checkEliminare eq = Right (Config restEqList substitutions)
-- | checkDescompunere eq = Right (Config ___ substitutions)



checkEliminare :: Equation -> Bool
checkEliminare ((Var x) :=: (Var y)) =  x == y
checkEliminare ((Var x) :=: (App func listeq)) = False
checkEliminare ((App func listeq) :=: (Var x)) = False

checkDescompunere :: Equation -> Bool
checkDescompunere ((App func1 listeq1) :=: (App func2 listeq2)) = func1 == func2 && length func1 == length func2
checkDescompunere ((Var x) :=: (Var y)) = False
checkDescompunere ((Var x) :=: (App func listeq)) = False
